    ...
    with torch.no_grad():
        ...
        for key in server_model.state_dict().keys():
            if 'num_batches_tracked' in key:
                ...
                cld_model.state_dict()[key].data.copy_( (1./len(args.hospital_id)) * (sum_theta + grad_theta) ) 

                server_model.state_dict()[key].data.copy_( (1./len(args.hospital_id)) * sum_theta )
