    ...
    with torch.no_grad():
        ...
        if args.algorithm == 'fedbn' or args.algorithm == "fedpxn":
            for key in server_model.state_dict().keys():
                if ('bn' not in key) and ('gn' not in key) :
                    temp = torch.zeros_like(server_model.state_dict()[key], dtype=torch.float32)

                    for client_idx in range(len(args.clients)):
    ...
    with torch.no_grad():
        ...
        for key in server_model.state_dict().keys():
            if 'num_batches_tracked' in key:
                ...
                cld_model.state_dict()[key].data.copy_( (1./len(args.clients)) * (sum_theta + grad_theta) ) 

                server_model.state_dict()[key].data.copy_( (1./len(args.clients)) * sum_theta )
